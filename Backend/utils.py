...
class MedicalDataValidator:
    ...
    @staticmethod
    def validate_temperature(temp, min_val=95.0, max_val=107.0):
        ...
        if medical_data.body_temperature:
            if medical_data.body_temperature >= config.TEMP_ALERT_HIGH:
                alert = Alert(
                    ...
                    message=f'High body temperature detected: {medical_data.body_temperature}°F (Fever)',
                    value=medical_data.body_temperature,
                    threshold=config.TEMP_ALERT_HIGH
                )
                db.session.add(alert)
                alerts_created.append(alert)
                logger.warning(f"High temperature alert for user {user_id}: {medical_data.body_temperature}°F")
                ...
                alert = Alert(
                    ...
                    severity='warning',
                    message=f'Low body temperature detected: {medical_data.body_temperature}°F',
                    value=medical_data.body_temperature,
                    threshold=config.TEMP_ALERT_LOW
                )
                db.session.add(alert)
                alerts_created.append(alert)
                logger.warning(f"Low temperature alert for user {user_id}: {medical_data.body_temperature}°F")
...
class DataExporter:
    ...
    def to_csv(medical_data_list):
        ...
        # CSV header
        csv_lines = [
            "ID,User ID,Date,Time,Body Temperature (°F),Pulse Rate (bpm),Heart Rate (bpm),SpO2 (%),Blood Pressure,Battery (%),Valid"
        ]
